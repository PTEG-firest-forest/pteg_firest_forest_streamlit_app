...
def plot_climate_variable_with_variance(
    ...
    df_plot["YearQuarter"] = (
        df_plot["Year"].astype(str) + "-Q" + df_plot["Quarter"].astype(str)
    )
    ...
    df_plot["Std_Dev"] = df_plot[values_list_column_name].apply(
    ...
    sns.lineplot(
        ...
        data=df_plot,
        marker="o",
        label=f"Promedio de {climate_variable_column_name}",
        ax=ax1,
        color="blue",
    )
    ...
    if not df_plot["Std_Dev"].isnull().all():
        ax1.fill_between(
            ...
            color="blue",
            alpha=0.2,
            label=f"{climate_variable_column_name} ± 1 Std. Dev.",
        )
    ...
    ax1.set_xticklabels(
        df_plot["YearQuarter"], rotation=90
...
@st.cache_data
def plot_quarterly_climate_comparison(
    df_resumen,
    years_to_compare,
    climate_variable_column_name="temperature_2m_mean_(C)",
    values_list_column_name="Values_List",
    ...
    for year in years_to_compare:
        for Q in range(1, 5):  # Iterate through quarters 1 to 4
            df_quarter_values = df_climate_values(
            ...
            if not df_quarter_values.empty:
                ...
                df_quarter_temp = df_quarter_values.rename(
                    columns={climate_col_name_dynamic: "Climate_Values"}
                )
    ...
    sns.boxplot(
        x="Quarter_Label",
        y="Climate_Values",
        data=df_comparacion_anual,
        hue="Quarter_Label",
        palette="viridis",
        legend=False,
    )
    plt.title(
        f"Comparación de la Distribución de Valores de {climate_variable_column_name} ({min(years_to_compare)}-Q1 a {max(years_to_compare)}-Q4)"
    )
...
@st.cache_data
def plot_all_climate_variables_time_series(
    df_input, title_suffix="Evolución a lo largo del tiempo", columns_to_plot=[]
):
    """
    ...
    df_hotspots = (
        df_working.groupby(["Year", "Quarter"]).size().reset_index(name="hotspot_count")
    )
    df_hotspots["YearQuarter"] = (
        df_hotspots["Year"].astype(str) + "-Q" + df_hotspots["Quarter"].astype(str)
    )
    ...
    ax_base.set_ylabel(
        "Cantidad de Hotspots (Registros)",
        color="#7f8c8d",
        fontweight="bold",
        fontsize=10,
    )
    ...
    for i, col_name in enumerate(selected_climate_variable_cols):
        ...
        df_summary_var = analyze_climate_by_quarter(
            df_input, climate_variable_column_name=col_name
        )

        if df_summary_var.empty:
            (line_placeholder,) = ax.plot(
                [], [], color=colors[i], linestyle="--", label=f"{col_name} (Sin Datos)"
            )
        ...
        df_plot["Std_Dev"] = df_plot["Values_List"].apply(
            lambda x: np.std(x) if len(x) > 1 else np.nan
        )
    ...
    plt.title(
        f"Evolución Climática vs Densidad de Hotspots\n{title_suffix}",
        fontsize=14,
        fontweight="bold",
        pad=20,
    )
...
def plot_hexagonal_map(
    ...
    """
    Genera y renderiza un mapa interactivo en Streamlit con hexágonos,
    donde la intensidad del color depende del campo seleccionado.
    ...
    width : int/str, optional
        Ancho del contenedor en Streamlit (None usa el ancho responsivo por defecto).
    height : int, optional
        Alto del mapa en píxeles (por defecto 500px).

    Returns:
    --------
    dict
        Diccionario devuelto por `st_folium` con la información de interacción del usuario.
    """
    if column not in _gdf.columns:
        st.error(f"Error: La columna '{column}' no se encuentra en el GeoDataFrame.")
        st.info(f"Columnas disponibles: {list(_gdf.columns)}")
        return None
    ...
    if steps is not None:
        extra_kwds["scheme"] = (
            "FisherJenks"  # Puedes usar 'FisherJenks', 'EqualInterval', 'Quantiles', etc.
        )
        extra_kwds["k"] = steps

    # Crear el mapa interactivo de Folium utilizando geopandas.explore()
    m = _gdf.explore(
        column=column,
        cmap=cmap,
    ...
    # Añadir el título dentro del HTML del mapa
    title_html = f"""
                 <h3 align="center" style="font-size:16px"><b>{title}</b></h3>
                 """
    m.get_root().html.add_child(folium.Element(title_html))

    # Renderizar el mapa en Streamlit usando st_folium
    map_data = st_folium(
        m,
        use_container_width=True,  # Ajusta el ancho automáticamente al contenedor de Streamlit
        height=height,
        key=f"hex_map_{column}",  # Clave única para evitar conflictos de estado al re-renderizar
    )
